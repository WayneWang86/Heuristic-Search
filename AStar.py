...
STUDENTS: IMPLEMENTATION OF AStar.py BY MODIFYING THE STARTER CODE
UCS.py FROM CSE 415.
...
print("\nWelcome to UCS, by Wayne Wang!")

h = Problem.h
COUNT = None  # Number of nodes expanded.
MAX_OPEN_LENGTH = None  # How long OPEN ever gets.
SOLUTION_PATH = []  # List of states from initial to goal, along lowest-cost path.
    ...
    #         and if it is already on CLOSED, delete the new instance.
        ...
        for op in Problem.OPERATORS:
            if op.precond(S):
                ...
                cost_update = cost + S.edge_distance(new_city) - h(S)
                new_f = cost_update + h(new_city)
                if new_city in CLOSED:
                    if new_f < g[new_city]:
                        CLOSED.remove(new_city)
                        OPEN.insert(new_city, new_f)
                        BACKLINKS[new_city] = S
                elif new_city in OPEN:
                    if new_f < OPEN[new_city]:
...
def backtrace(S):
    ...
    print(*path, sep=", ", end="")
    print(" ]")
    print ("Total Cost: " + str(float(TOTAL_COST)))
    return path
